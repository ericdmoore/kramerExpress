...
export function googleCalDest(astData: Promise<ICalendarAST[]>) {
  ...
  function formatedDate(date:Date, allDay:boolean = false) {
    const pad = (max:number, withStr:string) => (num:number) => String(num).padStart(max, withStr);
  ...
  return async (googleCalCFG: unknown) => {
     ...
     */
    const dates = `${formatedDate(ast[0].start, ast[0].isAllDay)}/${formatedDate(ast[0].end, ast[0].isAllDay)}`
     ...
     */
    const showFreeBusyOOO = ast[0].busy ? 'BUSY' : 'AVAILABLE' as 'AVAILABLE'| 'BUSY' | 'BLOCKING'
