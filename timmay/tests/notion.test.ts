...
const assertASTproperties = (ast: ICalendarAST) => {
  ...
  assert(
    ["private", ""].includes(ast.visibility),
    "AST should have a visibility flag",
  );
...
Deno.test({
  ...
  fn: async () => {
    const r = await notionDataFromDBid({
      DBiD: "77c40fcec736444bbdd036897c03bf81",
    })
      .to._data;
    r.forEach((data) => {
...
Deno.test({
  ...
  fn: async () => {
    const eventList = await notionData({
      id: "882526666b3a4e358d5021aa1d936a03",
    }).to._data;
    eventList.forEach((event) => {
      assertASTproperties(event);
    });
...
Deno.test({
  ...
  fn: async () => {
    ...
    for (const icsEvent of ics) {
      ...
      assert(
        icsEvent.description === rawEvent.description,
      ...
      assert(
        icsEvent.visibility === rawEvent.visibility,
        "Visibility should match",
      );
      assert(
        icsEvent.organizer === rawEvent.organizer,
